//Zlib.Gzip code https://github.com/imaya/zlib.js
    ...
    switch(e.data.command){
        ...
        */
        ...
        case 'record':
            ...
            break;
        case 'exportWAV':
            data=exportWAV(e.data.type);
            this.postMessage({data:data,eventId:e.data.eventId});
            break;
        case 'getBuffer':
            data=getBuffer();
            this.postMessage({data:data,eventId:e.data.eventId});
...
function getBuffer() {
    var buffers = [];
    buffers.push( mergeBuffers(recBuffersL) );
    if(outputChannels==2){
        buffers.push( mergeBuffers(recBuffersR) );
    }
    return buffers;
}

function getSendBuffer(isLast){
    ...
    if(isLast){
        jsonObj.idx="-"+jsonObj.idx;
    }
    var json=JSON.stringify(jsonObj);
    var gzip=new Zlib.Gzip(str2uint8(json));
    var jsonGzip=gzip.compress();

    if(sendOffset >= recBuffersL.length&&!isLast){
        sendIdx--;
        return null;
    }
    ...
    var buffer = new ArrayBuffer(jsonGzip.byteLength+4+audiodata.length*2+(CRLF.length+CRLF.length+cus_boundary.length)*3+2);
    var view = new DataView(buffer);
    var pos=0;
    pos=writeString(view, pos, CRLF+cus_boundary+CRLF);
    //pos=writeString(view, pos, json);
    pos=writeUint8Array(view, pos, jsonGzip);
    pos=writeString(view, pos, CRLF+cus_boundary+CRLF);
...
function mergeBuffers(recBuffers){
    var recLength=0;
    recBuffers.forEach(function(buffer){
    ...
    var sample_idx=0;
    var rate=sampleRate/outputSampleRate;
    var realLen=Math.ceil(recLength*outputSampleRate/sampleRate);
    var real_result = new Float32Array(realLen);
    for(i=0;i<realLen;i++){
        real_result[i]=result[Math.floor(sample_idx)];
        sample_idx+=rate;
    }
...
function writeUint8Array(view, offset, arr){
...
function writePcm(view, offset, buf){
    //var bufView=new DataView(buf);
    //for (var i = 0; i < buf.length; i++){
    //    view.setFloat32(offset + i*4,buf[i],true);
    //}
    floatTo16BitPCM(view, offset, buf);
    return offset+buf.length*2;
}
...
function encodeWAV(samples){
    ...
    view.setUint32(28, outputSampleRate, true);
    /* block align (channel count * bytes per sample) */
    view.setUint16(32, outputChannels * 2, true);
    /* bits per sample */
